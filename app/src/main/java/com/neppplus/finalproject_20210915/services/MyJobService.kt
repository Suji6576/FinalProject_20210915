package com.neppplus.finalproject_20210915.services
...
import com.neppplus.finalproject_20210915.receivers.AlarmReceiver
...
//        jobId => 약속정보 (출발/도착 좌표) => 상세정보확인 API통해 좌표정보도 받아오자.
...
//                예약된 작업에서 => 서버에 약속정보를 요청.
                if (response.isSuccessful) {

                    val basicResponse = response.body()!!
                    Log.d("응답확인", basicResponse.data.appointment.toString())

//                    서버가 알려준 약속데이터 => 교통정보 확인. => 알람 설정.
                    val appointmentData = basicResponse.data.appointment

                    val myODsayService = ODsayService.init(applicationContext, "jmqWEkgVT5Y4JjzCMbb7GdKtIJK1HhvO0YeG/PWfKAA")
...
//                                알람시간 : 약속시간(타임존에 맞게 변경) - 교통소요시간 - 내 준비시간 (밀리초단위 )
...
//    언제 알람을 울릴 지 계산해서 넘겨주면, 단순히 울리기만 하는 함수

    fun setAlarmByMilliSecond(timeInMillis: Long) {
//        알람을 울리게 도와주는 도구 => Broadcast 송신
        val alarmManager = getSystemService(ALARM_SERVICE) as AlarmManager

//        실제로 알람이 울릴 시간이 되면 실행할 코드 => BroadcastReceiver에 작업 해둘 필요 있다.
